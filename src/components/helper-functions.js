...
export function addRender(column) {
    if (column.format === "dec_0") {
        ...
        column.render = (text, row, index) =>
            !text ? text : <span>{`${text.toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 })}`}</span>;
    } else if (column.format === "dec_2") {
        column.render = (text, row, index) =>
            !text ? text : <span>{`${text.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}</span>;
    } else if (column.format === "percent") {
        column.render = (text, row, index) =>
            !text ? (
                text
            ) : (
                <span>{`${(text * 100).toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 })}%`}</span>
            );
        ...
        column.render = (text, row, index) =>
            !text || !playerMap[text] ? (
                ...
                <span>
                    ...
                    >
                        <Link target="_blank" href={`/player/${encodeURI(playerMap[text])}`}>
                            {text}
                        </Link>
                    </Tooltip>
                </span>
            ...
            !text || !teamMap[text] ? (
                ...
                <span>
                    <Tooltip
                        ...
                        overlayStyle={{ fontSize: "0.75rem" }}
                    >
                        <Link target="_blank" href={`/team/${encodeURI(teamMap[text])}`}>
                            {text}
                        </Link>
                    </Tooltip>
                </span>
...
/**
 * Given a tableData object and calcFields array, adds the calcFields to the tableData
 * @param {*} tableData
 * @param {*} calcsFields
 */
export function addCalcsToTable(tableData, calcsFields) {
    calcsFields.forEach((calc, index) => {
        tableData.columns.push(buildTableCalcColumn(`calc${index + 1}`, calc));
        tableData.dataSource.forEach((row) => (row[`calc${index + 1}`] = performCustomCalc(calc, row)));
    });
    return tableData;
}
