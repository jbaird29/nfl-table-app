...
import { UserOutlined, LoadingOutlined, MonitorOutlined } from "@ant-design/icons";
...
export default function LoadPage(props) {
    ...
    const filterRow = (row, statType) =>
        Object.fromEntries(Object.entries(row).filter(([dataIndex, value]) => shouldInclude(dataIndex, statType)));
        ...
        >
            ...
            <Card.Grid hoverable={false} style={{ padding: 6, width: "100%", textAlign: "left", lineHeight: 1.5 }}>
                <Paragraph style={paragraphStyle}>
                    <Text strong>Height:</Text> {info.height}
                </Paragraph>
                <Divider style={dividerStyle} />
                <Paragraph style={paragraphStyle}>
                    <Text strong>Weight:</Text> {info.weight}
                </Paragraph>
                <Divider style={dividerStyle} />
                <Paragraph style={paragraphStyle}>
                    <Text strong>Jersey:</Text> {info.jersey_number}
                </Paragraph>
                <Divider style={dividerStyle} />
                <Paragraph style={paragraphStyle}>
                    <Text strong>College:</Text> {info.college}
                </Paragraph>
                <Divider style={dividerStyle} />
                <Paragraph style={paragraphStyle}>
                    <Text strong>DOB:</Text> {info.birth_date.value} ({info.age})
                </Paragraph>
            </Card.Grid>
        </Card>
    );
    ...
    const getTeamCard = (info) => (
        ...
        >
            ...
            <Card.Grid hoverable={false} style={{ padding: 6, width: "100%", textAlign: "left", lineHeight: 1.5 }}>
                <Paragraph style={paragraphStyle}>
                    <Text strong>Coach:</Text> {info.coach}
                </Paragraph>
                <Divider style={dividerStyle} />
                <Paragraph style={paragraphStyle}>
                    <Text strong>GM:</Text> {info.general_manager}
                </Paragraph>
                <Divider style={dividerStyle} />
                <Paragraph style={paragraphStyle}>
                    <Text strong>Owners:</Text> {info.owners}
                </Paragraph>
                <Divider style={dividerStyle} />
                <Paragraph style={paragraphStyle}>
                    <Text strong>Venue:</Text> {info.venue}
                </Paragraph>
            </Card.Grid>
        </Card>
    );
        ...
        <Spin spinning={cardLoading}>
            <Row style={{ marginTop: 12 }}>
                <Button
                    style={{ width: 250, margin: "auto" }}
                    block
                    onClick={handleOpenCustomQueryClick}
                    shape="round"
                    icon={<MonitorOutlined />}
                >
                    Open in Custom Query
                </Button>
            </Row>
        </Spin>
    );
