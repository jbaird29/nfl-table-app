...
AS (
  SELECT
    ...
    CASE WHEN SPLIT(s.player_position, '/')[SAFE_OFFSET(0)] = 'HB' THEN 'RB' ELSE SPLIT(s.player_position, '/')[SAFE_OFFSET(0)] END AS player_position_history,
    FIRST_VALUE(CASE WHEN SPLIT(s.player_position, '/')[SAFE_OFFSET(0)] = 'HB' THEN 'RB' ELSE SPLIT(s.player_position, '/')[SAFE_OFFSET(0)] END) OVER (PARTITION BY s.player_id ORDER BY g.summary_season_year DESC, g.summary_week_title DESC) AS player_position,
    s.player_name || ' (' || FIRST_VALUE(CASE WHEN SPLIT(s.player_position, '/')[SAFE_OFFSET(0)] = 'HB' THEN 'RB' ELSE SPLIT(s.player_position, '/')[SAFE_OFFSET(0)] END) OVER (PARTITION BY s.player_id ORDER BY g.summary_season_year DESC, g.summary_week_title DESC) || ')' AS player_name_with_position,
    DENSE_RANK() OVER (ORDER BY s.player_id ASC) AS partition_player_id,
    CASE WHEN s.team_name = 'Redskins' THEN 'Football Team' ELSE s.team_name END AS team_name,
    s.team_id AS team_id,
    DENSE_RANK() OVER (ORDER BY CASE WHEN s.team_name = 'Redskins' THEN 'Football Team' ELSE s.team_name END ASC) AS partition_team_id,
